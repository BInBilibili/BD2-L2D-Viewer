  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" @click.self="onClose">
    <div class="w-full max-w-lg rounded-lg bg-gray-900 text-gray-100 shadow-xl border border-gray-700">
      <header class="flex items-center justify-between border-b border-gray-700 px-5 py-4">
        <h2 class="text-lg font-semibold">{{ t('upload.background.title') }}</h2>
        <button
          type="button"
      ...
      <div class="px-5 py-4 space-y-4">
        <p class="text-sm text-gray-300">{{ t('upload.background.description') }}</p>
        <label
        ...
        >
          <span class="text-base font-medium">{{ t('upload.background.selectImage') }}</span>
          <span class="text-xs text-gray-400">{{ t('upload.background.maxSizeHint') }}</span>
          <input type="file" accept="image/*" class="hidden" @change="onFileChange" />
        </label>
      </div>
      <footer class="flex items-center justify-end gap-3 border-t border-gray-700 px-5 py-4">
        <button class="text-sm text-gray-300 hover:text-white" @click="onReset" v-if="showReset">{{ t('upload.background.reset') }}</button>
